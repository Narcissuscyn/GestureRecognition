...
cv::Rect rightview_click_area[1]={cv::Rect(136,266,81,45)};


//1号摄像头里的全屏幕坐标区域
Rect screen1_area = cv::Rect(82,220,176,131);


//0号摄像头里的滑动坐标区域
Rect centerview_scale_area = cv::Rect(210,272,329,163);
...
//显示结果用
int show_result = 0,show_resultc = 0;
...
int JudgeScale(cv::Point p1,cv::Point p2)
{
        ...
        judge_scale==false)
    {
        ...
        {
            if(scale_dis[frame_now] - scale_dis[i] > 30)
            {
                if(frame_now - i > 3 && scale_dis[i]<90)
                {
                    int j=i;
                    while(j<frame_now)
                    {
        ...
        while(i<scale_dis.size()-1)
        {
            ...
            {
                if(frame_now - i > 3 && scale_dis[frame_now]<70)
                {
                    int j=i;
                    while(j<frame_now)
                    {
...
//    cv::setMouseCallback(windowName1, CallbackFunc1, NULL);
    ...
    int minH = 0, maxH = 20, minS = 60, maxS = 190, minV = 60, maxV = 255;
    int minH1 = 0, maxH1 = 50, minS1 = 30, maxS1 = 190, minV1 = 60, maxV1 = 255;
    cv::Mat frame;
    int counts = 0;
    cv::Mat frame1;
    int counts1 = 0;
...
//            cv::dilate(hsv, hsv, element);
            ...
            if (cv::contourArea(contours[largestContour]) > 2000) {
                    ...
                    if (hull[0].size() > 2) {
                        ...
                        for (size_t i = 0; i < convexityDefects.size(); i++) {
                            ...
                            if (inAngle > inAngleMin - 180 && inAngle < inAngleMax - 180 &&
                                length < lengthMax / 100.0 * boundingBox.height && length > 15 &&
...
///////////////////////////////////////////////////////////////////////////////////////////////
                            ...
                            if ((rightview_finger_series.size>=20)&&(!is_tar_box_null(tar_box)))
                            {
